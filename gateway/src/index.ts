...
// ============================================================================
// Internal Service Binding: Arena Evaluate
// ============================================================================

/**
 * Handle POST /safe-house/evaluate — only reachable via Cloudflare Service Binding.
 * The hostname 'internal' never resolves via DNS; this endpoint cannot be reached
 * from the public internet.
 *
 * Used by mnemom-api Arena V2 to run Safe House detection with fresh Supabase
 * pattern data (bypassing the KV cache that the normal gateway eval uses).
 */
async function handleInternalArenaEvaluate(request: Request, env: Env): Promise<Response> {
  try {
    ...
    const { payload, door, surface } = body;

    if (typeof payload !== 'string' || (door !== 'front' && door !== 'back')) {
      return Response.json({ error: 'Bad Request' }, { status: 400 });
    }

    if (door === 'front') {
      // ── Inbound (CFD) path ─────────────────────────────────────────────────
      // Fetch active threat patterns FRESH from Supabase (no KV cache —
      // Arena requires up-to-date data for accurate bypass detection).
      ...
      if (!patternsRes.ok) {
        return Response.json({ error: 'Failed to fetch threat patterns' }, { status: 502 });
      }
      const patterns = await patternsRes.json() as SafeHouseThreatPattern[];

      const { normalized } = preprocessForDetection(payload);
      const l1 = runL1Detection(normalized, patterns, {
        surface: surface as SourceType | undefined,
      });

      // Compute verdict from fixed thresholds
      let verdict: 'pass' | 'warn' | 'quarantine' | 'block';
      if (l1.score >= 0.95) {
        verdict = 'block';
      } else if (l1.score >= 0.8) {
        verdict = 'quarantine';
      } else if (l1.score >= 0.6) {
        verdict = 'warn';
      } else {
        verdict = 'pass';
      }

      // Serialize minhash: v1: followed by each uint32 as zero-padded 8-char hex
      const sig = computeMinHash(normalized);
      const minhash = `v1:${sig.map((n: number) => (n >>> 0).toString(16).padStart(8, '0')).join('')}`;

      return Response.json({
        verdict,
        detector_scores: {
          PatternMatcher: l1.score,
          SemanticAnalyzer: null,
        },
        detection_sources: l1.score > 0 ? ['PatternMatcher'] : [],
        threats: l1.threats as ThreatDetection[],
        minhash,
      });
    } else {
      // ── Outbound (CBD) path ────────────────────────────────────────────────
      const dlpMatches = scanDLP(payload);

      // Fetch active canaries from Supabase
      const canariesRes = await fetch(
        `${env.SUPABASE_URL}/rest/v1/sh_canaries?is_active=eq.true&select=keyword`,
        {
          headers: {
            apikey: env.SUPABASE_KEY,
            Authorization: `Bearer ${env.SUPABASE_KEY}`,
      ...
      if (!canariesRes.ok) {
        return Response.json({ error: 'Failed to fetch canaries' }, { status: 502 });
      }
      const canaries = await canariesRes.json() as { keyword: string }[];

      const payloadLower = payload.toLowerCase();
      const canaryHit = canaries.some(c => payloadLower.includes(c.keyword.toLowerCase()));

      const dlpScore = dlpMatches.length > 0 ? 0.9 : 0;
      const canaryScore = canaryHit ? 1.0 : 0;
      const overallRisk = Math.max(dlpScore, canaryScore);
      ...
      if (canaryScore > 0) detectionSources.push('CanaryMatcher');
      if (dlpScore > 0) detectionSources.push('DLPScanner');

      return Response.json({
        verdict,
        detector_scores: {
          CanaryMatcher: canaryScore || null,
          DLPScanner: dlpScore || null,
        },
        detection_sources: detectionSources,
        threats: [],
        minhash: null,
    ...
    console.error('[arena-evaluate] error:', err);
...
export default {
  async fetch(
    ...
    // Internal Service Binding route — only reachable from other CF Workers via Service Binding
    // (hostname 'internal' never resolves via DNS; this cannot be reached from the public internet)
    if (url.hostname === 'internal' && url.pathname === '/safe-house/evaluate') {
      return handleInternalArenaEvaluate(request, env);
    }
